...
class AutoGenAdapter(AgentFrameworkAdapter):
    """Adapter for AutoGen agents. Proxies all tool executions through AgentGuard."""
    ...
    async def execute_tool(self, agent_id: str, tool_name: str, tool_args: dict, context: dict | None = None) -> dict[str, Any]:
        if self._execute_fn:
            ...
            return {"framework": "autogen", "allowed": result.get("decision") == "ALLOWED", "result": result}
        return {"framework": "autogen", "allowed": False, "error": "No execution function configured"}

    async def validate_request(self, agent_id: str, request: dict) -> tuple[bool, str]:
        tool_name = request.get("function", {}).get("name", "")
        if not tool_name:
            return False, "No function name in AutoGen request"
        return True, "AutoGen request validated"
